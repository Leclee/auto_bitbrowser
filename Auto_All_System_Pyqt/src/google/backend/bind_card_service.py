...
async def auto_bind_card(page: Page, card_info: dict = None, account_info: dict = None, log_callback=None) -> Tuple[bool, str]:
    ...
    try:
        ...
        # ==================== Step 1: 点击 "Get student offer" / "畅享学生优惠" 按钮 ====================
        log("[BindCard] Step 1: 查找学生优惠按钮...")
        
        # 更精确的定位器：使用 jsname 属性（Google 按钮常用），同时支持中英文
        # 英文: "Get student offer", "Get offer"
        # 中文: "畅享学生优惠", "获取学生优惠", "学生优惠"
        get_offer_btn = page.locator(
            '[jsname="V67aGc"]:has-text("Get student offer"), '
            '[jsname="V67aGc"]:has-text("Get offer"), '
            '[jsname="V67aGc"]:has-text("畅享学生优惠"), '
            '[jsname="V67aGc"]:has-text("获取学生优惠"), '
            '[jsname="V67aGc"]:has-text("学生优惠")'
        )
        get_offer_btn_fallback = page.locator(
            'button:has-text("Get student offer"), '
            'button:has-text("Get offer"), '
            'button:has-text("畅享学生优惠"), '
            'button:has-text("获取学生优惠"), '
            'button:has-text("学生优惠")'
        )
        ...
        if has_subscribe and (has_change_payment or not has_add_card):
            ...
            try:
                ...
                if subscribe_result == 'error':
                    ...
                    # Step B: 重新点击 "Get student offer" / "畅享学生优惠"
                    try:
                        # 支持中英文按钮
                        combined_offer = page.locator(
                            'button:has-text("Get student offer"), '
                            'button:has-text("Get offer"), '
                            'button:has-text("畅享学生优惠"), '
                            'button:has-text("获取学生优惠"), '
                        ...
                        await combined_offer.first.click()
                        log("[BindCard] ✅ 重新点击学生优惠按钮")
                        await asyncio.sleep(1)
                    except Exception as e:
                        log(f"[BindCard] ⚠️ 重新点击学生优惠按钮失败: {e}")
